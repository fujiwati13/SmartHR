...
#End Region
    ...
    Protected Sub BindData(ByVal CmdWhere As String)
        ...
        Using con As New SqlConnection(constr)
            Using cmd As New SqlCommand("spPagingReimburseType")
                cmd.Connection = con
    ...
    Private Sub dtgList_ItemCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles dtgList.ItemCommand
        ...
        Select Case e.CommandName
            ...
            Case "Delete"
                ...
                Try
                    Using con As New SqlConnection(constr)
                        Using cmd As New SqlCommand("spDeleteReimburseType")
                            Using sda As New SqlDataAdapter()
                                cmd.CommandType = CommandType.StoredProcedure
                                cmd.Parameters.AddWithValue("@RaimburseID", lblRaimburseID.Text.Trim())
                                cmd.Connection = con
                                con.Open()
                                cmd.ExecuteNonQuery()
                                con.Close()
    ...
    Private Sub imbSearch_Click(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imbSearch.Click
        ...
        If txtSearch.Text <> "" Then
            If ddlSearch.SelectedItem.Value.Trim = "RaimburseID" Then
                If CBool(InStr(txtSearch.Text, "%")) Then
                    CmdWhere = CmdWhere & CmdWhere & "RaimburseID like '" & txtSearch.Text.Trim & "' "
                Else
                    CmdWhere = CmdWhere & "RaimburseID = '" & txtSearch.Text.Trim & "' "
                End If
            ElseIf ddlSearch.SelectedItem.Value.Trim = "RaimburseDesc" Then
                If Right(txtSearch.Text.Trim, 1) = "%" Then
                    CmdWhere = CmdWhere & "RaimburseDesc like '" & txtSearch.Text.Trim & "' "
                Else
                    CmdWhere = CmdWhere & "RaimburseDesc like '" & txtSearch.Text.Trim & "' "
                End If
            End If
    ...
    Protected Sub btnsave_Click(sender As Object, e As EventArgs)
        Try
            Using con As New SqlConnection(constr)
                Using cmd As New SqlCommand("spSaveAddEditReimburseType")
                    Using sda As New SqlDataAdapter()
                        cmd.CommandType = CommandType.StoredProcedure
                        cmd.Parameters.AddWithValue("@RaimburseID", txtRaimburseID.Text.Trim())
                        cmd.Parameters.AddWithValue("@RaimburseDesc", txtDescription.Text.Trim())
    ...
    Protected Sub btnCancel_Click(sender As Object, e As EventArgs)
        Reset()
        Response.Redirect("ReimburseType.aspx")
    End Sub
