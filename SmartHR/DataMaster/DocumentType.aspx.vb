...
#End Region
    ...
    Protected Sub BindData(ByVal CmdWhere As String)
        ...
        Using con As New SqlConnection(constr)
            Using cmd As New SqlCommand("spPagingDocumentType")
                cmd.Connection = con
    ...
    Private Sub dtgList_ItemCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles dtgList.ItemCommand
        ...
        Select Case e.CommandName
            Case "Edit"
                ...
                txtDocumentID.Text = lblDocumentID.Text
            ...
            Case "Delete"
                ...
                Try
                    Using con As New SqlConnection(constr)
                        Using cmd As New SqlCommand("spDeleteDocumentType")
                            Using sda As New SqlDataAdapter()
                                cmd.CommandType = CommandType.StoredProcedure
                                cmd.Parameters.AddWithValue("@DocumentID", lblDocumentID.Text.Trim())
                                cmd.Connection = con
                                con.Open()
                                cmd.ExecuteNonQuery()
                                con.Close()
    ...
    Private Sub imbSearch_Click(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imbSearch.Click
        ...
        If txtSearch.Text <> "" Then
            ...
            End If
            'If ddlSearch.SelectedItem.Value.Trim = "DocumentID" Then
            '    If CBool(InStr(txtSearch.Text, "%")) Then
            '        CmdWhere = CmdWhere & CmdWhere & "DocumentID like '" & txtSearch.Text.Trim & "' "
            '    Else
            '        CmdWhere = CmdWhere & "DocumentID = '" & txtSearch.Text.Trim & "' "
            '    End If
            'ElseIf ddlSearch.SelectedItem.Value.Trim = "DocumentDesc" Then
            '    If Right(txtSearch.Text.Trim, 1) = "%" Then
            '        CmdWhere = CmdWhere & "DocumentDesc like '" & txtSearch.Text.Trim & "' "
            '    Else
            '        CmdWhere = CmdWhere & "DocumentDesc like '" & txtSearch.Text.Trim & "' "
            '    End If
            'End If
    ...
    Protected Sub btnsave_Click(sender As Object, e As EventArgs)
        Try
            Using con As New SqlConnection(constr)
                Using cmd As New SqlCommand("spSaveAddEditDocumentType")
                    Using sda As New SqlDataAdapter()
                        cmd.CommandType = CommandType.StoredProcedure
                        cmd.Parameters.AddWithValue("@DocumentID", txtDocumentID.Text.Trim())
                        cmd.Parameters.AddWithValue("@DocumentDesc", txtDescription.Text.Trim())
