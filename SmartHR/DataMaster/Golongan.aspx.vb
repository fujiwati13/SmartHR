...
#End Region
    ...
    Protected Sub BindData(ByVal CmdWhere As String)
        ...
        Using con As New SqlConnection(constr)
            Using cmd As New SqlCommand("spPagingGolongan")
                cmd.Connection = con
...
#End Region
    ...
    Private Sub dtgList_ItemCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles dtgList.ItemCommand
        ...
        Select Case e.CommandName
            Case "Edit"
                ...
                txtGolonganID.Text = lblGolonganID.Text
            ...
            Case "Delete"
                ...
                Try
                    Using con As New SqlConnection(constr)
                        Using cmd As New SqlCommand("spDeleteGolongan")
                            Using sda As New SqlDataAdapter()
                                cmd.CommandType = CommandType.StoredProcedure
                                cmd.Parameters.AddWithValue("@GolonganID", lblGolonganID.Text.Trim())
                                cmd.Connection = con
                                con.Open()
                                cmd.ExecuteNonQuery()
                                con.Close()
    ...
    Private Sub imbSearch_Click(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imbSearch.Click
        ...
        If txtSearch.Text <> "" Then
            ...
            End If
            'If ddlSearch.SelectedItem.Value.Trim = "GolonganID" Then
            '    If CBool(InStr(txtSearch.Text, "%")) Then
            '        CmdWhere = CmdWhere & CmdWhere & "GolonganID like '" & txtSearch.Text.Trim & "' "
            '    Else
            '        CmdWhere = CmdWhere & "GolonganID = '" & txtSearch.Text.Trim & "' "
            '    End If
            'ElseIf ddlSearch.SelectedItem.Value.Trim = "GolonganName" Then
            '    If Right(txtSearch.Text.Trim, 1) = "%" Then
            '        CmdWhere = CmdWhere & "GolonganName like '" & txtSearch.Text.Trim & "' "
            '    Else
            '        CmdWhere = CmdWhere & "GolonganName like '" & txtSearch.Text.Trim & "' "
            '    End If
            'End If
    ...
    Protected Sub btnsave_Click(sender As Object, e As EventArgs)
        Try
            Using con As New SqlConnection(constr)
                Using cmd As New SqlCommand("spSaveAddEditGolongan")
                    Using sda As New SqlDataAdapter()
                        cmd.CommandType = CommandType.StoredProcedure
                        cmd.Parameters.AddWithValue("@GolonganID", txtGolonganID.Text.Trim())
                        cmd.Parameters.AddWithValue("@GolonganName", txtDescription.Text.Trim())
    ...
    Protected Sub btnCancel_Click(sender As Object, e As EventArgs)
        Reset()
        Response.Redirect("Golongan.aspx")
    End Sub
